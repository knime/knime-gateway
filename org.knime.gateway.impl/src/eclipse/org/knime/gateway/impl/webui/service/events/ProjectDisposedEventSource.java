...
/**
 * Even source emitting {@link ProjectDisposedEventEnt}s.
 *
...
public final class ProjectDisposedEventSource
    ...
    public ProjectDisposedEventSource(final EventConsumer eventConsumer, final ProjectManager projectManager) {
        super(eventConsumer);
        Consumer<String> listener = id -> {
            if (m_projectIds.contains(id)) {
                sendEvent(builder(ProjectDisposedEventEntBuilder.class).setProjectId(id).build(),
                    id);
                m_projectIds.remove(id);
            }
        };
        projectManager.addProjectRemovedListener(listener);
    }
