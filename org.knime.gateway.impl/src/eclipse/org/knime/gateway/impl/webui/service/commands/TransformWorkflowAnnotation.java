...
class TransformWorkflowAnnotation extends AbstractWorkflowCommand {
    ...
    protected boolean executeWithLockedWorkflow() throws OperationNotAllowedException {
        ...
        m_annotationId = DefaultServiceUtil.entityToAnnotationID(getWorkflowKey().getProjectId(), m_commandEnt.getId());
        var annotation = wfm.getWorkflowAnnotations(m_annotationId)[0];
        if (annotation == null) {
            throw new OperationNotAllowedException("No workflow annotation found for id " + m_commandEnt.getId());
        }
        m_previousBounds =
