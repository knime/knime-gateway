...
public class NodeDataTestHelper extends AbstractGatewayServiceTestHelper {
     ...
     */
    public void testCompareNodeData() throws Exception {
        ...
        BufferedDataTable table = getDataTable("3", 1);
    ...
    /**
     * Compares a {@link BufferedDataTable} to a {@link DataTableEnt} cell-wise by deserializing the cells from the
     * respective entities.
     *
     ...
     */
    private static void compare(final BufferedDataTable table, final DataTableEnt tableEnt, final int startIdx) {
        assertThat("Total row count doesn't match", tableEnt.getNumTotalRows(), is(table.size()));

        try (CloseableRowIterator rowIt = table.iterator()) {
     ...
     * @return
     */
    private BufferedDataTable getDataTable(final String nodeID, final int portIdx) {
