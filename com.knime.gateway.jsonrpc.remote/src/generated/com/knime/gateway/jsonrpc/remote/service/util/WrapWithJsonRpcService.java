...
import com.knime.gateway.jsonrpc.remote.service.JsonRpcStaticNodeServiceWrapper;
import com.knime.gateway.v0.service.StaticNodeService;
import com.knime.gateway.jsonrpc.remote.service.JsonRpcExecEnvServiceWrapper;
...
public class WrapWithJsonRpcService {
    ...
    public static GatewayService wrap(final GatewayService service, final Class<?> serviceInterface) {
        try {
            ...
            if(serviceInterface == StaticNodeService.class) {
                return JsonRpcStaticNodeServiceWrapper.class.getConstructor(serviceInterface).newInstance(service);
            }
            if(serviceInterface == ExecEnvService.class) {
                return JsonRpcExecEnvServiceWrapper.class.getConstructor(serviceInterface).newInstance(service);
            }
