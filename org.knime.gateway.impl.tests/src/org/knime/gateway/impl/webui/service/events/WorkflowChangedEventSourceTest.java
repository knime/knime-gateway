...
public class WorkflowChangedEventSourceTest {
    ...
    public void testClearDependentNodePropertiesCacheOnListenerRemoval() throws IOException, IllegalArgumentException,
        ...
        wfm.paste(content);
        projectManager.addProject(DefaultProject.builder(wfm).setId("1").build());

        // create event source, add and remove event listener
        var workflowMiddleware = new WorkflowMiddleware(projectManager, null);
        var eventSource = new WorkflowChangedEventSource(mock(EventConsumer.class), workflowMiddleware, projectManager);
        var wfKey = new WorkflowKey("1", NodeIDEnt.getRootID());
        ...
        var event = eventSource.addEventListenerAndGetInitialEventFor(eventType);
        ...
        event = eventSource.addEventListenerAndGetInitialEventFor(eventType);
